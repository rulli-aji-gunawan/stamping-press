...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const hiddenTotalProdTimeInput = document.getElementById('total_prod_time'); // gunakan input hidden yang sudah ada

  if (!startTimeInput || !finishTimeInput || !showTotalProdTimeInput || !hiddenTotalProdTimeInput) return;

  // Gunakan event 'input' agar realtime
  startTimeInput.addEventListener('input', calculateTotalProductionTime);
  finishTimeInput.addEventListener('input', calculateTotalProductionTime);

  function calculateTotalProductionTime() {
    const startTime = startTimeInput.value;
    const finishTime = finishTimeInput.value;

    // Cek jika salah satu kosong, kosongkan hasil
    ...
    let finishDate = new Date(0, 0, 0, finishHour, finishMinute, 0);

    // Jika finish lebih kecil dari start, berarti lewat tengah malam
    ...
    showTotalProdTimeInput.value = diffMins + ' minutes';
    hiddenTotalProdTimeInput.value = diffMins;
  }
});
...
function fetchDowntimeClassifications() {
  fetch('/master-data/downtime-classification/all', {
    ...
    .then(response => {
      ...
      // Cek content-type untuk mencegah HTML diparse sebagai JSON
    ...
    .catch(error => {
      ...
      downtimeClassifications = []; // Set default value
      return [];
    });
}
...
// let rowCount = 0;
let rowCount = $('#tbl-prod-problem tbody tr').length || 0;
...
function addRow() {
  ...
  let newRow = `
    <tr id="row${rowCount}">
        ...
        <td class="time-problem">
          <input type="number" id="total-problem-time" name="production_problems[${rowCount}][total_time]" readonly>
        </td>
...
function removeRow(btn) {
  // btn adalah elemen button, cari tr terdekat lalu hapus
...
function saveData() {
  ...
  $('#tbl-prod-problem tbody tr').each(function () {
    ...
    if (validateProblemData(problem)) {
      tableDowntimes.push(problem);
    }
  });

  if (tableDowntimes.length === 0) {
  ...
  console.log('Data yang akan dikirim:', tableDowntimes);
  ...
  $.ajax({
    ...
    data: JSON.stringify({
      table_downtimes: tableDowntimes
    }),
...
document.querySelector('.report-form').addEventListener('submit', function (e) {
  ...
  // Tambahkan ini untuk menunjukkan bahwa form sedang diproses
